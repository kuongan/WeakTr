...
# Load pretrained RPN (COCO) — outside of train_one_epoch



def train_one_epoch(model: torch.nn.Module, data_loader: Iterable,
                    optimizer: torch.optim.Optimizer, device: torch.device,
                    epoch: int, loss_scaler=None, max_norm: float = 0,
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        # === Step 1: Get proposals from RPN ===
        with torch.no_grad():
            proposals_batch = rpn_model(samples) 
            proposals_list = [p['boxes'] for p in proposals_batch]  

        all_rois = []
        roi_targets = []

        for img, boxes, label in zip(samples, proposals_list, targets):
            for box in boxes:
                x1, y1, x2, y2 = map(int, box.tolist())
                if (x2 - x1 < 10 or y2 - y1 < 10): 
                    continue
                roi = img[:, y1:y2, x1:x2]
                roi = TF.resize(roi, [224, 224])
                all_rois.append(roi)
                roi_targets.append(label)  

        if len(all_rois) == 0:
            continue 

        roi_batch = torch.stack(all_rois).to(device)
        roi_targets = torch.stack(roi_targets).to(device)

        # === Step 2: Forward through WeakTr ===
        with torch.cuda.amp.autocast():
            outputs, patch_outputs, attn_outputs = model(roi_batch)

            loss = F.multilabel_soft_margin_loss(outputs, roi_targets)
            metric_logger.update(cls_loss=loss.item())

            ploss = F.multilabel_soft_margin_loss(patch_outputs, roi_targets)
            metric_logger.update(pat_loss=ploss.item())
            loss = loss + ploss

            aloss = F.multilabel_soft_margin_loss(attn_outputs, roi_targets)
...
def evaluate(data_loader, model, device):
    ...
    for images, target in metric_logger.log_every(data_loader, 10, header):
        ...
        batch_size = images.shape[0]
                # === Step 1: Get proposals from RPN ===
        with torch.no_grad():
            proposals_batch = rpn_model(images) 
            proposals_list = [p['boxes'] for p in proposals_batch]  

        all_rois = []
        roi_targets = []

        for img, boxes, label in zip(images, proposals_list, target):
            for box in boxes:
                x1, y1, x2, y2 = map(int, box.tolist())
                if (x2 - x1 < 10 or y2 - y1 < 10): 
                    continue
